...
for train_data_name in args.train_data_names:
    ...
    for index, row in data.iterrows():
        try:
            uuid = row["uuid"]
            label = row["label"]
            feature = torch.load(f"{args.train_data_dir}/{train_data_name}/{uuid}.pt").flatten()
...
output_size = 1
model = SimpleFC(input_size, args.hidden_sizes, output_size, dropout_prob=args.dropout_prob, verbose = args.print_network_layout)
model.train()
model.to(device)
...
if args.dont_save: # Save the model
